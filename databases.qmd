...
That said, we're not going to start with SQL, but instead we'll teach you dbplyr, which can translate your dplyr code to SQL.
...
-   In SQL, case doesn't matter: you can write `select`, `SELECT`, or even `SeLeCt`. In this book we'll stick with the common convention of writing SQL keywords in uppercase to distinguish them from table or variable names.
...
This is one of the idiosyncrasies of SQL: `WHERE` is evaluated before `SELECT` and `GROUP BY`, so SQL needs another clause that's evaluated afterwards.
...
If you hit this scenario, the [dm package](https://cynkra.github.io/dm/), by Tobias Schieferdecker, Kirill Müller, and Darko Bergant, is a lifesaver.
...
2.  Explain what each of the following SQL queries does and try recreate them using dbplyr.
