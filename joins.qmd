...
For example, there are two ways to join the `flights2` and `airports` table: either by `dest` or `origin`:
...
-   A **right join** keeps all observations in `y`, @fig-join-right.
    ...
    #|   appears in the output. 
    #| fig-alt: |
    #|   Compared to the previous diagram showing a left join, the x table
...
In equi joins the `x` keys and `y` keys are always equal, so we only need to show one in the output.
...
#|   of x matches one row of y and the second and third rows each match
