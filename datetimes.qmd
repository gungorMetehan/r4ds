...
```{r}
#| fig.alt: >
#|   A frequency polygon with departure time (Jan-Dec 2013) on the x-axis
...
#|   on weekends. There are few days that stand out as having a surprisingly
...
#|   flights, ~48,000, decreasingly slightly over the course of the week.
...
```{r}
#| fig-alt: |
#|   A line plot with departure time on the x-axis. This is units of seconds
...
```{r}
#| fig-alt: |
#|   A line plot with departure time (midnight to midnight) on the x-axis
...
You've seen them in a few places and have no doubt encountered in your own analysis, and it's now time to provide a grab bag of useful techniques for dealing with them.
