...
1.  Create a new Quarto document using *File \> New File \> Quarto Document*.
    ...
    Then render the document by clicking the appropriate button and then by using the appropriate keyboard shortcut.
...
You can either paste an image from your clipboard directly into the visual editor (and RStudio will place a copy of that image in the project directory and link to it) or you can use the visual editor's Insert \> Figure / Image menu to browse to the image you want to insert or paste its URL.
...
You can also edit Quarto documents using the Source editor in RStudio, without the assistance of the Visual editor.
...
3.  Copy and paste the contents of `diamond-sizes.qmd` from <https://github.com/hadley/r4ds/tree/main/quarto> into a local R Quarto document.
...
3.  Modify `diamond-sizes.qmd` to use `label_comma()` to produce nicely formatted output.
...
1.  [DOI](https://quarto.org/docs/visual-editor/technical.html#citations-from-dois) (Digital Object Identifier) references.
