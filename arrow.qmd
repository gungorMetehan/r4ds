...
I highly recommend using `curl::multi_download()` to get very large files as it's built for exactly this purpose: it gives you a progress bar and it can resume the download if it's interrupted.
...
-   The parquet format improves performance by storing data in a binary format that can be read more directly into memory. The column-wise format and rich metadata mean that arrow only needs to read the four columns actually used in the query (`CheckoutYear`, `MaterialType`, `CheckoutMonth`, and `Checkouts`).
