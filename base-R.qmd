...
You'll undoubtedly encounter these other approaches when you start reading R code written by others, particularly if you're using Stack Overflow.
...
This chapter focuses on base R, so doesn't have any real prerequisites, but we'll load the tidyverse in order to explain some of the differences.
...
If `df` is a `data.frame`, then `df[, cols]` will return a vector if `cols` selects a single column and a data frame if it selects more than one column.
