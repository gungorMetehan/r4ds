...
@fig-bool-ops shows examples of commonly used Boolean operations and how they work.
...
#| out-width: NULL
#| fig-cap: | 
#|    A set of commonly used Boolean operations. `x` is the left-hand
...
One of the most powerful features of logical vectors is their use for conditional transformations, i.e. doing one thing for condition x, and something different for condition y.
...
You'll always use the first three arguments of `if_else()`. The first argument, `condition`, is a logical vector, the second, `true`, gives the output when the condition is true, and the third, `false`, gives the output if the condition is false.
...
4.  Write a `case_when()` statement that uses the `month` and `day` columns from `flights` to label a selection of important US holidays (e.g., New Year's Day, 4th of July, Thanksgiving, and Christmas).
