...
#|   second child is a list. A rightward facing triangle indicates that the
...
#| fig.alt: >
#|   Another screenshot of the list-viewer having expanded the second
...
As suggested, let's use `names_sep` to resolve the problem:
...
2.  The `owner` column of `gh_repos` contains a lot of duplicated information because each owner can have many repos.
...
For example, the repurrrsive package also provides the source for `gh_user` as a JSON file and you can read it with `read_json()`:
